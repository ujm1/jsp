...
function start(port = 0, host = '127.0.0.1', timeout = SHUTDOWN_TIMEOUT) {
    return new Promise(resolve => {
        (0, helpers_1.debug)(`starting the bridge server at port ${port}`);
        ...
        app.post('/close', (_request, response) => {
            (0, helpers_1.debug)('the bridge server will shutdown');
            response.end(() => {
                server.close();
            });
        });
        server.on('close', () => {
            (0, helpers_1.debug)('the bridge server closed');
            orphanTimeout.stop();
        });
        server.on('error', (err) => {
            (0, helpers_1.debug)(`the bridge server error: ${err}`);
        });
        server.on('listening', () => {
            var _a;
            /**
             ...
             */
            (0, helpers_1.debug)(`the bridge server is running at port ${(_a = server.address()) === null || _a === void 0 ? void 0 : _a.port}`);
            resolve(server);
        });
